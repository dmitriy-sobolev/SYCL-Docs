...
vec<float,1> fmax(vec<float,1> x, vec<float,1> y)
vec<float,2> fmax(vec<float,2> x, vec<float,2> y)
vec<float,3> fmax(vec<float,3> x, vec<float,3> y)
...
* The following generic type names sometimes appear together in the same
  function description:
+
--
** [code]#vfloatn#
...
--
+
When this occurs, the replacement of the generic type names are correlated.
If one of these generic type names is replaced with a [code]#vec# of length
_Len_, then all other occurrences of these generic type names in that same
function description must also be replaced with a [code]#vec# of length _Len_.
For example, if a function takes one parameter of type [code]#vfloatn# and
another of type [code]#vfloatnptr#, there are overloads like:
+
[source]
----
func(vec<float,1>, multi_ptr<vec<float,1>, Space, IsDecorated>)
func(vec<float,2>, multi_ptr<vec<float,2>, Space, IsDecorated>)
// ...
----
+
However, there are no overloads where the two parameters have different
vector lengths.

* Some of the generic type names can be replaced with a templated type, where
  the identifiers [code]#N#, [code]#Space#, and [code]#IsDecorated# represent
  template parameters.  When this happens, the overload is a templated
  function, as shown above with the [code]#fmax# example.  Only a few
...
Where the [code]#N# template parameter is constrained to the values 2, 3 or 4.
...
unsigned long
unsigned long long
vec<int8_t,1>   vec<int8_t,2>   vec<int8_t,3>
vec<int8_t,4>   vec<int8_t,8>   vec<int8_t,16>
vec<int16_t,1>  vec<int16_t,2>  vec<int16_t,3>
vec<int16_t,4>  vec<int16_t,8>  vec<int16_t,16>
vec<int32_t,1>  vec<int32_t,2>  vec<int32_t,3>
vec<int32_t,4>  vec<int32_t,8>  vec<int32_t,16>
vec<int64_t,1>  vec<int64_t,2>  vec<int64_t,3>
vec<int64_t,4>  vec<int64_t,8>  vec<int64_t,16>
vec<uint8_t,1>  vec<uint8_t,2>  vec<uint8_t,3>
vec<uint8_t,4>  vec<uint8_t,8>  vec<uint8_t,16>
vec<uint16_t,1> vec<uint16_t,2> vec<uint16_t,3>
vec<uint16_t,4> vec<uint16_t,8> vec<uint16_t,16>
vec<uint32_t,1> vec<uint32_t,2> vec<uint32_t,3>
vec<uint32_t,4> vec<uint32_t,8> vec<uint32_t,16>
vec<uint64_t,1> vec<uint64_t,2> vec<uint64_t,3>
vec<uint64_t,4> vec<uint64_t,8> vec<uint64_t,16>
marray<char,N>
...
vec<uint64_t,1> vec<uint64_t,2> vec<uint64_t,3>
vec<uint64_t,4> vec<uint64_t,8> vec<uint64_t,16>
vec<float,1>    vec<float,2>    vec<float,3>
vec<float,4>    vec<float,8>    vec<float,16>
vec<double,1>   vec<double,2>   vec<double,3>
vec<double,4>   vec<double,8>   vec<double,16>
vec<half,1>     vec<half,2>     vec<half,3>
...
[source]
----
multi_ptr<int, Space, IsDecorated>
----
...
[source]
----
multi_ptr<float, Space, IsDecorated>
----
...
[source]
----
multi_ptr<double, Space, IsDecorated>
----
...
[source]
----
multi_ptr<half, Space, IsDecorated>
----
...
[source]
----
multi_ptr<vec<float,1>, Space, IsDecorated>
multi_ptr<vec<float,2>, Space, IsDecorated>
multi_ptr<vec<float,3>, Space, IsDecorated>
multi_ptr<vec<float,4>, Space, IsDecorated>
multi_ptr<vec<float,8>, Space, IsDecorated>
multi_ptr<vec<float,16>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<vec<double,1>, Space, IsDecorated>
multi_ptr<vec<double,2>, Space, IsDecorated>
multi_ptr<vec<double,3>, Space, IsDecorated>
multi_ptr<vec<double,4>, Space, IsDecorated>
multi_ptr<vec<double,8>, Space, IsDecorated>
multi_ptr<vec<double,16>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<vec<half,1>, Space, IsDecorated>
multi_ptr<vec<half,2>, Space, IsDecorated>
multi_ptr<vec<half,3>, Space, IsDecorated>
multi_ptr<vec<half,4>, Space, IsDecorated>
multi_ptr<vec<half,8>, Space, IsDecorated>
multi_ptr<vec<half,16>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<marray<float,N>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<marray<double,N>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<marray<half,N>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<marray<int,N>, Space, IsDecorated>
----
...
[source]
----
multi_ptr<vec<int32_t,1>, Space, IsDecorated>
multi_ptr<vec<int32_t,2>, Space, IsDecorated>
multi_ptr<vec<int32_t,3>, Space, IsDecorated>
multi_ptr<vec<int32_t,4>, Space, IsDecorated>
multi_ptr<vec<int32_t,8>, Space, IsDecorated>
multi_ptr<vec<int32_t,16>, Space, IsDecorated>
----
